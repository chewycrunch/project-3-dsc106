<script lang="ts">
	...
	function getYDomain(): [number, number] {
		return useFahrenheit ? [toFahrenheit(35.25), toFahrenheit(39)] : [35.25, 39];
	}
	...
	async function loadAndProcessData() {
		const response = await fetch((!dev ? '/project-3-dsc106' : '') + '/mouse_data.csv');
	...
	function createVisualization() {
		...
		const xDomain = currentBrushSelection ? currentBrushSelection : [0, 14]; // default to full range

		const xScale = d3.scaleLinear().domain(xDomain).range([0, innerWidth]);

		const yScale = d3.scaleLinear().domain(getYDomain()).range([innerHeight, 0]);
		...
		const vizGroup = g.append('g').attr('class', 'visualization');

		// Create background group for light/dark periods
		const bgGroup = vizGroup.append('g').attr('class', 'background');
		...
		for (let day = 0; day < 14; day++) {
			...
			bgGroup
				...
				.attr(
					'width',
					xScale(Math.min(day + 1, xDomain[1])) - xScale(Math.max(day + 0.5, xDomain[0]))
				)
				.attr('height', innerHeight)
				.attr('fill', '#FFFFEC')
				.attr('opacity', 1);
		...
		if (showEstrus && dataType === 'female') {
			const estrusGroup = vizGroup.append('g').attr('class', 'estrus');
			...
			[2, 6, 10].forEach((day) => {
				if (day + 1 >= xDomain[0] && day <= xDomain[1]) {
					estrusGroup
						...
						.attr(
							'width',
							xScale(Math.min(day + 1, xDomain[1])) - xScale(Math.max(day, xDomain[0]))
						)
						.attr('height', innerHeight)
		...
		const visibleData = processedData.filter((d) => d.day >= xDomain[0] && d.day <= xDomain[1]);
		...
		vizGroup
           ...
           Period: ${d.light ? 'Lights Off' : 'Lights On'}` +
			...
			.on('mouseout', function () {
				d3.select(this).style('opacity', 0).attr('r', 2);
	...
	function createBrushVisualization() {
		...
		if (currentBrushSelection) {
			brushG.call(brush.move, [xScale(currentBrushSelection[0]), xScale(currentBrushSelection[1])]);
		}
	...
	<div class="legend">
		...
		<div class="legend-items">
			...
			{#if dataType === 'female'}
				<div class="legend-item">
					<div
						class="legend-color estrus-color"
						style="background-color: #FCCFCE; opacity: 1;"
					></div>
					<span>Estrus</span>
				</div>
			{/if}
		...
		<div class="controls">
			...
			<button class="reset-zoom" on:click={resetView}> Reset View </button>
		</div>
	</div>
</div>
